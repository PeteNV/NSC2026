...
for th_name in cons:
    if th_name not in th2en:
        continue
    en_name = th2en[th_name]
    l_kwh = cons[th_name].get(LOW, 0)
    h_kwh = cons[th_name].get(HIGH, 0)
    l_users = users[th_name].get(LOW, 0)
    h_users = users[th_name].get(HIGH, 0)
    total_users = l_users + h_users
    if total_users == 0:
        continue
    total_kwh = l_users * l_kwh + h_users * h_kwh
    monthly = total_kwh / total_users / 12
    results[en_name] = round(monthly, 2)

print('// Auto-generated from PEA electricity data (B.E. 2565)')
print('export const PROVINCIAL_MONTHLY_KWH: Record<string, number> = {')
for name in sorted(results.keys()):
    print(f'  "{name}": {results[name]},')
print('};')
print(f'// {len(results)} provinces')
